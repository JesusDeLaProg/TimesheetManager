...
} from '../autocomplete-select/autocomplete-select.component';
        ...
        <table mat-table [dataSource]="data()">
            ...
            <td mat-cell *matCellDef="let line">
              <div class="project-cell">
                ...
                <tm-project-autocomplete-select
                  [(value)]="line.project"
                  (valueChange)="projectChanged($event)"
                />
              </div>
            </td>
            <td
              class="last-sticky-cell"
              [ngClass]="
                ((containerScroll | async) ?? 0) > 0 ? 'mat-elevation-z4' : ''
              "
              colspan="3"
              *matFooterCellDef
              style="background-color: white; padding-left: 16px; padding-right: 16px;"
            >
              <div style="align-content: flex-end" class="project-footer">
                <b>Total:</b>
              </div>
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="activity" sticky>
            ...
            <td
              ...
              *matCellDef="let line"
              style="width: 110px"
            >
              <tm-activity-autocomplete-select
                [disabled]="!line.project"
                [chosenPhase]="line.phase"
                [(value)]="line.activity"
              />
            </td>
            <td *matFooterCellDef></td>
          ...
          @for (col of daysColumns; track col; let i = $index) {
            <ng-container matColumnDef="{{ col }}">
              ...
              <td class="day-total" *matFooterCellDef>
                <b>{{ getTotalHourForDay(i) }}</b>
              </td>
            </ng-container>
          }
          ...
          <ng-container matColumnDef="week-total-0">
            ...
            <td style="text-align: center;" mat-cell *matCellDef="let line">
              <b *ngIf="line.project">{{ getWeekTotal(0, line) }}</b>
            </td>
          ...
          <ng-container matColumnDef="week-total-1">
            <th mat-header-cell *matHeaderCellDef></th>
            <td style="text-align: center;" mat-cell *matCellDef="let line">
              <b *ngIf="line.project">{{ getWeekTotal(1, line) }}</b>
            </td>
            <td style="text-align: center;" mat-footer-cell *matFooterCellDef>
              <b>{{ getWeekTotal(1) }}</b>
            </td>
          ...
          <ng-container matColumnDef="total">
            ...
            <td style="text-align: center;" mat-cell *matCellDef="let line">
              <b *ngIf="line.project">{{ getLineTotal(line) }}</b>
            </td>
            <td style="text-align: center;" mat-footer-cell *matFooterCellDef>
              <b>{{ getTimesheetTotal() }}</b>
            </td>
          </ng-container>
...
export class TimesheetTableComponent implements OnInit {
  ...
  dates = computed(() => {
    ...
    if (t) {
      ...
      return Array(this.DAYS_COUNT)
        .fill(0)
        .map((_, i) => start.plus({ day: i }).startOf('day').toJSDate());
    }
    return [];
  });
  headerColumns = ['project', 'phase', 'activity', 'divers'];
  daysColumns = Array(this.DAYS_COUNT)
    .fill(0)
    .map((_, i) => `entry-${i}`);
  columnsToDisplay = (() => {
  ...
  containerScroll?: Observable<number>;

  ngOnInit(): void {
    this.containerScroll = fromEvent<Event>(
  ...
  getTotalHourForDay(index: number) {
    return this._timesheet()
      ?.lines.map((l) => l.entries[index])
      .reduce((total, e) => total + e.time, 0);
  }
  ...
  getWeekTotal(i: number, line?: ITimesheetLine): number {
    ...
    return (
      this._timesheet()?.lines.reduce(
        (v, l) => v + this.getWeekTotal(i, l),
        0,
      ) ?? 0
    );
  }
  ...
  getTimesheetTotal(): number {
    return (
      this._timesheet()?.lines.reduce(
        (total, l) => total + this.getLineTotal(l),
        0,
      ) ?? 0
    );
  }
    ...
    if (newValue && this._timesheet()?.lines.every((l) => !!l.project)) {
      this._timesheet.update((t) => {
        t?.lines.push({
          ...
          phase: '',
          activity: '',
          entries: Array(this.DAYS_COUNT)
            .fill({})
            .map((_, i) => ({
              date: DateTime.fromJSDate(t.begin)
                .startOf('day')
                .plus({ days: i })
                .toJSDate(),
              time: 0,
