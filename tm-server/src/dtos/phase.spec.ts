...
describe('PhaseDTO', () => {
    ...
    it('is valid', () => {
      expect(validator.validate({ _id: '1', code: 'AB', name: 'test', activities: ['2', '3'] })).resolves.toEqual<ValidationResult<Phase>>({
        __success: true,
        _id: '1',
        code: 'AB',
        name: 'test',
    ...
    it('is empty object and invalid', () => {
      expect(validator.validate({})).resolves.toMatchObject<ValidationResult<Phase>>({
        __success: false,
        errors: [
            { property: 'code', constraints: { isString: 'code doit être du texte', matches: 'code doit respecter le format /[A-Z]{2,3}/' } },
            { property: 'name', constraints: { isString: 'name doit être du texte', isNotEmpty: 'name ne doit pas être vide' } },
            { property: 'activities', constraints: { isString: 'activities doit être du texte', isNotEmpty: 'activities ne doit pas être vide' } }
        ]
      })
    });
  
    it('is missing code and invalid', () => {
      expect(validator.validate({ name: 'Abcd', activities: ['2', '3'] })).resolves.toMatchObject<ValidationResult<Phase>>({
        __success: false,
        errors: [
          { property: 'code', constraints: { isString: 'code doit être du texte', matches: 'code doit respecter le format /[A-Z]{2,3}/' } },
        ]
    ...
    it('is missing name and invalid', () => {
      expect(validator.validate({ code: 'AB', activities: ['2', '3'] })).resolves.toMatchObject<ValidationResult<Phase>>({
        __success: false,
        errors: [
          { property: 'name', constraints: { isString: 'name doit être du texte', isNotEmpty: 'name ne doit pas être vide' } }
        ]
    ...
    it('has invalid code', () => {
      expect(validator.validate({ code: '1ab' })).resolves.toMatchObject<ValidationResult<Phase>>({
        __success: false,
        errors: [
          { property: 'code', constraints: { matches: 'code doit respecter le format /[A-Z]{2,3}/' } },
          { property: 'name', constraints: { isString: 'name doit être du texte', isNotEmpty: 'name ne doit pas être vide' } },
          { property: 'activities', constraints: { isString: 'activities doit être du texte', isNotEmpty: 'activities ne doit pas être vide' } }
        ]
      })
    ...
    it('has string activities and invalid', () => {
      expect(validator.validate({ code: 'AB', name: 'test', activities: '2' })).resolves.toMatchObject<ValidationResult<Phase>>({
        __success: false,
        errors: [
          { property: 'activities', constraints: { isArray: 'activities doit être une liste' } }
        ]
      })
    });
